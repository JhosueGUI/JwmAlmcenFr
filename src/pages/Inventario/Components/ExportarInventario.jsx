...
export function ExportarInventario() {
    const exportarExcel = async () => {
        try {
            ...
            const response = await axios.get('https://zoological-blessing-production.up.railway.app/api/orden_compra/exportar_inventario', {
                responseType: 'blob' // Para manejar archivos binarios (Excel en este caso)
            });
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `inventario-${fechaFormateada}.xlsx`); // Nombre del archivo con fecha actual
