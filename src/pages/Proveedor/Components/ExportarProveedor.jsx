...
export function ExportarProveedor() {
    const exportarExcel = async () => {
        try {
            const fecha = new Date();
            const fechaFormateada = fecha.toISOString().split('T')[0]; // Formato YYYY-MM-DD

            const response = await axios.get('https://zoological-blessing-production.up.railway.app/api/orden_compra/exportar_proveedor', {
                responseType: 'blob' // Para manejar archivos binarios (Excel en este caso)
            });
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `proveedor-${fechaFormateada}.xlsx`); // Nombre del archivo con fecha actual
